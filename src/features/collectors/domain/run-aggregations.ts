...
function completedRuns(runs: CollectorRun[]): CollectorRun[] {
  return runs.filter((run) => run.durationMs !== null);
}

export function successRate(runs: CollectorRun[]): number {
...
export function avgDurationMs(runs: CollectorRun[]): number {
  const completed = completedRuns(runs);
  if (completed.length === 0) {
    return 0;
  }
...
export function p95DurationMs(runs: CollectorRun[]): number {
...
export function lastErrorMessage(runs: CollectorRun[]): string | null {
  const failed = runs.find((run) => run.success === false && run.errorMessage);
  return failed?.errorMessage ?? null;
}

export function runsWithinHours(
  runs: CollectorRun[],
  hours: number,
): CollectorRun[] {
...
export function totalMessagesPublished(runs: CollectorRun[]): number {
